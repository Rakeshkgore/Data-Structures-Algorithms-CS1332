 ...
 * Collaborators: N/A
 *
 * Resources: Piazza, Powerpoint Slides, CS vis tool
 */
public class ExternalChainingHashMap<K, V> {
    ...
    public V put(K key, V value) {
        ...
        V returnedVal;
        ExternalChainingMapEntry<K, V> newValue = new ExternalChainingMapEntry<K, V>(key, value);
        ...
        if (table[index] == null) {
            ...
            while (current.getNext() != null && !current.getKey().equals(key)) {
                ...
                returnedVal = current.getValue();
                current.setValue(value);
                return returnedVal;
            } else {
                ExternalChainingMapEntry<K, V> newCurrent = new ExternalChainingMapEntry(key, value, table[index]);
        ...
        if (table[index] == null) {
            ...
            while (current != null) {
                if (current.getNext().getKey().equals(key)) {
                    returnedVal = current.getNext().getValue();
     ...
     */
    public Set<K> keySet() {
        HashSet<K> keysList = new HashSet<>(size);

        for (ExternalChainingMapEntry<K, V> current : table) {
            if (current != null) {
    ...
    public List<V> values() {
        List<V> keysList = new ArrayList<V>(size);

        for (ExternalChainingMapEntry<K, V> current: table) {
            if (current != null) {
                keysList.add(current.getValue());
                while (current.getNext() != null) {
                    current = current.getNext();
                    keysList.add(current.getValue());
                }
    ...
    public void resizeBackingTable(int length) {
        if (length < size || length < 0) {
            throw new IllegalArgumentException("Length should be greater than the size or 0");
        }
        ExternalChainingMapEntry<K, V>[] values = new ExternalChainingMapEntry[length];
        for (ExternalChainingMapEntry value : table) {
            if (value != null) {
                int index = Math.abs(value.getKey().hashCode());
                values[index] = value;
            }
        }
        table = values;
